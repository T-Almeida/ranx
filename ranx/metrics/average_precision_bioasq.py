...
# LOW LEVEL FUNCTIONS ==========================================================
@njit(cache=True)
def _average_precision_bioasq(qrels, run, k, rel_lvl):
    ...
    return np.sum(precision_scores) / np.min(np.array([qrels.shape[0], np.int64(10)]))



@njit(cache=True, parallel=True)
def _average_precision_bioasq_parallel(qrels, run, k, rel_lvl):
    scores = np.zeros((len(qrels)), dtype=np.float64)
    for i in prange(len(qrels)):
        scores[i] = _average_precision_bioasq(qrels[i], run[i], k, rel_lvl)
    return scores


# HIGH LEVEL FUNCTIONS =========================================================
def average_precision_bioasq(
    ...
    return _average_precision_bioasq_parallel(qrels, run, k, rel_lvl)
